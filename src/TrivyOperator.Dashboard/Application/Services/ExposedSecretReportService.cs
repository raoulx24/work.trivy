...
public class ExposedSecretReportService(IConcurrentCache<string, IList<ExposedSecretReportCr>> cache)
    : IExposedSecretReportService
{
        ...
        IEnumerable<int>? excludedSeverities = null)
    {
        ...
        IEnumerable<ExposedSecretReportDto> dtos = cache
            ...
            .SelectMany(
                kvp => kvp.Value.Select(cr => cr.ToExposedSecretReportDto())
                    .Select(
                        dto =>
                        {
        ...
        IEnumerable<int>? excludedSeverities = null)
    {
        ...
        IEnumerable<ExposedSecretReportImageDto> exposedSecretReportImageDtos = cache
            .Where(kvp => string.IsNullOrEmpty(namespaceName) || kvp.Key == namespaceName)
            .SelectMany(
                kvp => kvp.Value.GroupBy(esr => esr.Report?.Artifact?.Digest)
