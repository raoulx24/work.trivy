...
public class ClusterComplianceReportService(IConcurrentCache<string, IList<ClusterComplianceReportCr>> cache) : IClusterComplianceReportService
{
    public Task<IEnumerable<ClusterComplianceReportDto>> GetClusterComplianceReportDtos()
    {
        IEnumerable<ClusterComplianceReportDto> value = cache
    ...
    {
        IEnumerable<ClusterComplianceReportDenormalizedDto> value = cache
            .SelectMany(kvp => kvp.Value)
