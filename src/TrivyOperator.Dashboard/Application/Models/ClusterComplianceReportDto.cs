...
public class ClusterComplianceReportDto
{
    ...
    public long TotalPassCount { get; init; } = 0;
    public long TotalFailCount { get; init; } = 0;
    public long TotalFailCriticalCount { get; init; } = 0;
    public long TotalFailHighCount { get; init; } = 0;
    public long TotalFailMediumCount { get; init; } = 0;
    public long TotalFailLowCount { get; init; } = 0;
...
{
    public static ClusterComplianceReportDto ToClusterComplianceReportDto(this ClusterComplianceReportCr clusterComplianceReportCr)
    {
        ClusterComplianceReportDetailDto[] details = clusterComplianceReportCr.Spec.Compliance.Controls
            .Select(control => new ClusterComplianceReportDetailDto
            {
                Id = control.Id,
                Name = control.Name,
                ...
                TotalFail = clusterComplianceReportCr.Status.SummaryReport.ControlCheck
                    .Where(x => x.Id == control.Id).FirstOrDefault()?.TotalFail ?? 0,
            }).ToArray();
        ...
        return new ClusterComplianceReportDto
        {
            ...
            RelatedResources = clusterComplianceReportCr.Spec.Compliance.RelatedResources.Select(x => x.ToString()).ToArray(),
            ...
            TotalFailCriticalCount = details.Where(x => x.SeverityId == (int)TrivySeverity.CRITICAL && x.TotalFail > 0).Count(),
            TotalFailHighCount = details.Where(x => x.SeverityId == (int)TrivySeverity.HIGH && x.TotalFail > 0).Count(),
            TotalFailMediumCount = details.Where(x => x.SeverityId == (int)TrivySeverity.MEDIUM && x.TotalFail > 0).Count(),
    ...
    public static IList<ClusterComplianceReportDenormalizedDto> ToClusterComplianceReportDenormalizedDto(this ClusterComplianceReportCr clusterComplianceReportCr)
    {
        return clusterComplianceReportCr.Spec.Compliance.Controls
            .Select(control => new ClusterComplianceReportDenormalizedDto
            {
                Name = clusterComplianceReportCr.Metadata.Name,
                Uid = new Guid(clusterComplianceReportCr.Metadata.Uid),
                Description = clusterComplianceReportCr.Spec.Compliance.Description,
